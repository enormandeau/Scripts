...
class blastplus():
    ...
    def is_good(self, blast_id):
        ...
        (strand, begin, end)
        """
        max_blast_results = 5 # Max num of blast results per search in raw blast file
        self.result_names = []
...
def get_raw_blast(sequences, raw_blasts_file):
    ...
    """
    for i in [l for l in blast_results if '***** No hits found *****' not in l]:
        print "####################################################################"
        print
        for j in i:
            print j
...
def get_fasta(sequences, fasta_file):
    ...
    while end != True: ### Use a for loop instead!!!
        ...
        if name in sequences:
            ...
            seq.good_sequence = seq.sequence[begin:end]
            # TESTING --v
            """
            print
            print name
            print "Begin:", begin, "End:", end
            print "Difference:", end - begin
            print len(seq.sequence), "--", seq.sequence
            print len(seq.good_sequence), "--", seq.good_sequence
            if end - begin == len(seq.good_sequence):
                print "OK"
            else:
